...
# === 文件路径设置 ===
csv_file_name = "log_2"  # 替换为你的 CSV 文件名
...
def plot_position_data(x_positions, y_positions):
    ...
    # 绘制轨迹线
        # 绘制轨迹线（深绿色）
        ax.plot(x_positions, y_positions, color='darkgreen', linewidth=3)


    # 保存为 SVG 矢量图，透明背景
    plt.savefig("trajectory_plot_2_2.svg", format="svg", transparent=True, bbox_inches='tight')
    plt.close()
    print("Trajectory plot saved as 'trajectory_plot_2_2.svg'")
