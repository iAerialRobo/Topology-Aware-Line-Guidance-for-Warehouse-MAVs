...
#YAW_TOLERANCE_PX = 10
FOCAL_LENGTH_PX = 407  # 相机焦距（像素）

def calculate_yaw_angle(deviation, altitude_m, max_yaw_angle=30):
    """
    计算偏航角（单位：度）

    Args:
        deviation (float): 图像横向偏差（像素）
        altitude_m (float): 无人机当前高度（米）
        max_yaw_angle (float): 限制的最大偏航角度（度）
    ...
    # 限制偏航角范围
    yaw_angle_clamped = max(-max_yaw_angle, min(yaw_angle_deg, max_yaw_angle))

    return yaw_angle_clamped
...
yaw_threshold = 45 # 自定义阈值  # 每一轮自定义阈值
...
directory_4 = '/home/dym/course4_data_4'
data_file_path_4 = os.path.join(directory_4, 'log_4.csv')
# 检查并创建目录
if not os.path.exists(directory_4):
    os.makedirs(directory_4)
# 初始化计时器标志
start_timer_4_initialized = False
start_time_4 = None
# 初始化 CSV 文件并写入表头（含单位）
with open(data_file_path_4, 'w', newline='') as csvfile_4:
    csv_writer_4 = csv.writer(csvfile_4)
    csv_writer_4.writerow([
        "Timestamp (s)", "Position_X (m)", "Position_Y (m)", "Position_Z (m)",
        "Speed_X (m/s)", "Speed_Y (m/s)", "Speed_Z (m/s)",
        "Pitch (rad)", "Roll (rad)", "Yaw (rad)", "Image_Center_Offset (pixels)"
    ])
...
def image_callback_1(msg):
    ...
    # === 图像保存 ===
    folder_path = '/home/dym/course1_PID_images'
    os.makedirs(folder_path, exist_ok=True)
    image_filename = f"image_{image_counter}.jpg"
    image_counter += 1
    cv2.imwrite(os.path.join(folder_path, image_filename), small_image)
...
def image_callback_2(msg):
    ...
    # === 图像保存 ===
    folder_path = '/home/dym/course2_PID_images'
    os.makedirs(folder_path, exist_ok=True)
    image_filename = f"image_{image_counter}.jpg"
    image_counter += 1
    cv2.imwrite(os.path.join(folder_path, image_filename), small_image)
...
def image_callback_4(msg):
    global camera_subscriber, t_left_count, last_shape_code, pause_movement, exit_flag, image_counter
    global tracking_active, tracking_state
    global start_timer_4_initialized, start_time_4
    global current_pose, current_twist, current_pitch, current_roll, current_yaw
    global pid_phase_started, pid_phase_completed, first_nonzero_code_seen, center_tracking_enabled
    global t_marker_phase_ready
    global second_pid_phase_started, second_pid_phase_completed
    ...
    if not start_timer_4_initialized:
        start_time_4 = time.time()
        start_timer_4_initialized = True
        print("Timer initialized for image_callback_4.")

    timestamp = time.time() - start_time_4
    ...
    # === 图像保存 ===
    folder_path = '/home/dym/course4_PID_images'
    os.makedirs(folder_path, exist_ok=True)
    image_filename = f"image_{image_counter}.jpg"
    image_counter += 1
    cv2.imwrite(os.path.join(folder_path, image_filename), small_image)
    ...
    # === CSV 记录 ===
    yellow_line_offset = 0
    with open(data_file_path_4, 'a', newline='') as csvfile_4:
        csv_writer_4 = csv.writer(csvfile_4)
        csv_writer_4.writerow([
            f"{round(timestamp, 2)} s",
            f"{position.x:.2f} m" if position else "N/A",
